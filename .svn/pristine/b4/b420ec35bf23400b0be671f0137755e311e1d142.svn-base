...
-(void) saveDownloadedImage :(UIImage *)image withURL:imageURL
{
	if(image !=nil)
	{
		...
		NSString *pathToSaveImages=[Utility retrievePathForNameInDocumentDirectory :[imageURL lastPathComponent]];
...
{
	UIImage *image=[UIImage imageWithContentsOfFile:[Utility retrievePathForNameInDocumentDirectory :[imgURL lastPathComponent]]];//loading image if already present
	return image;
}
